...
const FCommunity = () => {
    ...
    useEffect(() => {
        ...
        if (email && !Number.isNaN(lat) && !Number.isNaN(lon)) {
            fetch(`http://localhost:5000/api/chat/nearby-users?email=${encodeURIComponent(email)}&lat=${lat}&lon=${lon}`)
                .then((res) => res.json())
            ...
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                fetch(`http://localhost:5000/api/chat/nearby-users?email=${encodeURIComponent(email)}&lat=${position.coords.latitude}&lon=${position.coords.longitude}`)
                    .then((res) => res.json())
                    .then((data) => setNearbyUsers(data.users || []))
                    .catch(() => setNearbyUsers([]));
            });
