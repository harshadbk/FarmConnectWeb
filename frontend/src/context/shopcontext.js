...
const ShopContextProvider = (props) => {
  ...
  useEffect(() => {
    fetch('http://13.233.124.185:5000/allproducts')
      .then((resp) => {
        if (!resp.ok) {
        ...
        const normalized = (Array.isArray(data) ? data : []).map((p, idx) => {
          ...
          const image = p.image
            ? String(p.image).startsWith('http')
              ? p.image
              : `http://13.233.124.185:5000/${String(p.image).replace(/^\/+/, '')}`
            : '';
    ...
    if (localStorage.getItem('auth-token')) {
      fetch('http://13.233.124.185:5000/getcart', {
        method: 'POST',
        headers: {
  ...
  const addToCart = (itemId) => {
    ...
    if (localStorage.getItem('auth-token')) {
      fetch('http://13.233.124.185:5000/addtocart', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'auth-token': localStorage.getItem('auth-token'),
  ...
  const removeFromCart = async (itemId) => {
    try {
      ...
      const response = await fetch('http://13.233.124.185:5000/removefromcart', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
